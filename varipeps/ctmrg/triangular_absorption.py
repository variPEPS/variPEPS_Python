...
from .triangular_projectors import (
    ...
    calc_T_30_150_270_truncated,
    calc_T_90_210_330_truncated,
...
def decompose_new_T(new_T, chi, truncation_eps, config, only_trunc=False):
...
def do_absorption_step_triangular(
    ...
    for x, iter_rows in working_unitcell.iter_all_rows(only_unique=True):
        for y, view in iter_rows:
            ...
            new_T1_full.append(new_T1 / jnp.linalg.norm(new_T1))
            ...
            new_T2_full.append(new_T2 / jnp.linalg.norm(new_T2))
            new_T2a_trunc, new_T2b_trunc = decompose_new_T(
                new_T2, working_tensor_obj.chi, truncation_eps, config, True
            )
            ...
            new_T3_full.append(new_T3 / jnp.linalg.norm(new_T3))
            ...
            new_T4_full.append(new_T4 / jnp.linalg.norm(new_T4))
            new_T4a_trunc, new_T4b_trunc = decompose_new_T(
                new_T4, working_tensor_obj.chi, truncation_eps, config, True
            )
            ...
            new_T5_full.append(new_T5 / jnp.linalg.norm(new_T5))
            ...
            new_T6_full.append(new_T6 / jnp.linalg.norm(new_T6))
            new_T6a_trunc, new_T6b_trunc = decompose_new_T(
                new_T6, working_tensor_obj.chi, truncation_eps, config, True
            )
    ...
    for x, iter_rows in working_unitcell.iter_all_rows(only_unique=True):
        for y, view in iter_rows:
            ...
            new_t.T3_full = new_T3_full[i]
            new_t.T4_full = new_T4_full[i]
            # new_t.T3b = None
            # new_t.T4a = None
    ...
    for x, iter_rows in working_unitcell.iter_all_rows(only_unique=True):
        for y, view in iter_rows:
            (T1a, T1b), (T3a, T3b), (T5a, T5b), smallest_S_T = (
                calc_T_90_210_330_truncated(
            ...
            new_t_00 = view[0, 0][0][0].copy_including_trunc()
            new_t_00.T1b = _post_process_CTM_tensors(T1b, view[0, 0][0][0].T1b, config)
            new_t_00.T3b = _post_process_CTM_tensors(T3b, view[0, 0][0][0].T3b, config)
            new_t_00.T5a = _post_process_CTM_tensors(T5a, view[0, 0][0][0].T5a, config)
            view[0, 0] = new_t_00

            new_t_01 = view[0, 1][0][0].copy_including_trunc()
            new_t_01.T1a = _post_process_CTM_tensors(T1a, view[0, 1][0][0].T1a, config)
            view[0, 1] = new_t_01

            new_t_10 = view[1, 0][0][0].copy_including_trunc()
            new_t_10.T3a = _post_process_CTM_tensors(T3a, view[1, 0][0][0].T3a, config)
            view[1, 0] = new_t_10

            new_t_11 = view[1, 1][0][0].copy_including_trunc()
            new_t_11.T5b = _post_process_CTM_tensors(T5b, view[1, 1][0][0].T5b, config)
            view[1, 1] = new_t_11
    ...
    for x, iter_rows in working_unitcell.iter_all_rows(only_unique=True):
        for y, view in iter_rows:
            (T2a, T2b), (T4a, T4b), (T6a, T6b), smallest_S_T = (
                calc_T_30_150_270_truncated(
                    *_get_triangular_ctmrg_2x2_structure(peps_tensors, view),
                    config,
                    state,
            ...
            new_t_00 = view[0, 0][0][0].copy_including_trunc()
            new_t_00.T2b = _post_process_CTM_tensors(T2b, view[0, 0][0][0].T2b, config)
            new_t_00.T4a = _post_process_CTM_tensors(T4a, view[0, 0][0][0].T4a, config)
            new_t_00.T6a = _post_process_CTM_tensors(T6a, view[0, 0][0][0].T6a, config)
            view[0, 0] = new_t_00

            new_t_01 = view[0, 1][0][0].copy_including_trunc()
            new_t_01.T4b = _post_process_CTM_tensors(T4b, view[0, 1][0][0].T4b, config)
            view[0, 1] = new_t_01

            new_t_10 = view[1, 0][0][0].copy_including_trunc()
            new_t_10.T6b = _post_process_CTM_tensors(T6b, view[1, 0][0][0].T6b, config)
            view[1, 0] = new_t_10

            new_t_11 = view[1, 1][0][0].copy_including_trunc()
            new_t_11.T2a = _post_process_CTM_tensors(T2a, view[1, 1][0][0].T2a, config)
            view[1, 1] = new_t_11
