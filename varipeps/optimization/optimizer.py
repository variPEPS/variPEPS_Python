...
def optimize_peps_network(
    ...
    while count < varipeps_config.optimizer_max_steps:
        ...
        logger.info(
            "📉 Step %d | Energy: %.8f Δ: %.2e | Retries: %d | Conv: %.3e | Line search step: %s | Max. trunc. err.: %.3e | Runtime: %.0f sec",
            int(count),
            float(working_value),
        ...
        if (
            ...
            if remaining_slurm_time < time_of_one_step:
                logger.info("⏳ Average time of optimizer step below remaining Slurm runtime")
                if (
